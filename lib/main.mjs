...
async function _installStyleFileFallback({ styleXML, styleID, filename, force = false }) {
  ...
  if (destFile.exists() && !force) {
    try { Zotero.debug(`[IndigoBook CSL-M] style fallback skipped (file exists): ${filename}`); } catch (e) {}
...
async function _installStyleIfMissing({ rootURI, dataStore, relPath }) {
  ...
  if (installedStyle) {
    ...
    if (!shouldUpdate) {
      try { Zotero.debug(`[IndigoBook CSL-M] style already up to date: ${styleID}`); } catch (e) {}
      return;
    }

    try { Zotero.debug(`[IndigoBook CSL-M] updating installed style: ${styleID}`); } catch (e) {}
  }
...
async function _ensureBundledStylesInstalled({ rootURI, dataStore }) {
  ...
  try {
    ...
    try { Zotero.debug(`[IndigoBook CSL-M] style install skipped (styles/index.json unavailable): ${String(e)}`); } catch (_) {}
    return;
  }

  if (!Array.isArray(files) || !files.length) {
    try { Zotero.debug('[IndigoBook CSL-M] style install skipped (styles/index.json empty or invalid)'); } catch (e) {}
    return;
  }

export class PrefsUI {
  ...
  async _tryRegister() {
    try {
      ...
      try { Zotero.debug(`[Citation Phoenix] prefs pane register failed: ${String(e)}`); } catch (_) {}
  ...
  _scheduleRetry(reason) {
    if (this._registerAttempts >= this._maxRegisterAttempts) {
      try { Zotero.debug(`[Citation Phoenix] prefs pane registration gave up after ${this._registerAttempts} attempts: ${reason}`); } catch (_) {}
      return;
    }

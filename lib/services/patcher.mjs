  ...
  _patchItemPaneRender() {
    ...
    itemDetails.render = async function (...args) {
      try {
        const itemID = this.item?.id;
        if (itemID != null) {
          try { Zotero.debug(`[Citation Phoenix] case reporter item-pane render sync: item=${String(itemID)}`); } catch (e) {}
          await self._syncItemFromFieldsAndMLZ(itemID);
        }
      } catch (e) {
        try { Zotero.debug(`[Citation Phoenix] case reporter item-pane render sync failed: ${String(e)}`); } catch (_) {}
      }
      ...
      try {
        ...
        const lookupNoHints = category === 'title'
          ...
          : category === 'container-title'
            ? !self._shouldAllowContainerTitleFallback(key)
            : noHints;
